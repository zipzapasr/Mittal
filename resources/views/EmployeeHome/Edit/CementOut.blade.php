...
@section('content')
    ...
    @php
        $cement_outsCount = $cement_outs->count();
    @endphp
    ...
    <div class="container" style="margin-top:70px;">
        ...
        <div class="row mt-3">
            <h5 style="font: bold;">Edit Cement Outs
                Date: {{ $date }}
            </h5>
            <form method="POST" action="{{route('employee.saveEdits', ['key' => 3, 'site' => $site->id, 'date' => $date])}}" id="form"> {{--action="{{$site->id}}/{{$date}}"--}}
                @csrf
                @if( $errors)
                ...
                <table class="table container table-responsive">
                    ...
                    <tbody class="appendedRow">
                        @foreach($cement_outs as $k => $cement_out)
                            @php
                                $ind = $k;
                            @endphp
                            <tr>
                                <td style="text-transform: capitalize;">
                                    <input type="number" min="0" class="form-control" name="bags[]" value="{{ $cement_out->bags}}" id="bags" placeholder="Num. of Bags" required>
                                </td>
                                @if($errors->has('bags'))
                                ...
                                <td style="text-transform: capitalize;padding-top: 20px;" class="">
                                    <select class="form-control" name="from_site[]">
                                        @foreach($mysites as $site)
                                            <option value="{{ $site->id }} {{($site->id == $cement_out->from_site) ? 'selected' : ''}}">{{ $site->site_name }}</option>
                                        @endforeach
                                    </select>
                                </td>
                                ...
                                <td>
                                    <select class="form-control" name="to_site[]">
                                        @foreach($allsites as $site)
                                            <option value="{{ $site->id }} {{($site->id == $cement_out->to_site) ? 'selected' : ''}}">{{ $site->site_name }}</option>
                                        @endforeach
                                    </select>
                                </td>
                ...
                <div class="row">
                    {{-- <div class="mb_30 col-md-6" style="float: left">
                        <button type="submit" id="save" class="btn btn-primary btn-hidden" {{ ($cement_outsCount == 0) ? 'hidden' : '' }}>Save</button>
                    </div> --}}
...
@section('javascript')
    ...
    <script>
        ...
        $(document).ready(function() {
            var cement_insCount = parseInt("<?php echo($cement_outsCount) ?>");
