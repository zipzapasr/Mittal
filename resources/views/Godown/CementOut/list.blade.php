                        ...
                        <tbody>
                            @foreach($cementOuts as $cementOut)
                                <tr>
                                    ...
                                    <td>
                                        @if ($cementOut->from_site_id == 0)
                                            Godown
                                        @else
                                            {{ $cementOut->getFromSite->site_name }}
                                        @endif
                                    </td>
                                    <td>{{ $cementOut->getToSite->site_name }}</td>
                                    <td>{{ $cementOut->remark }}</td>
                                    <td>
                                        <a href="{{ route('edit.godown.cementOut' , ['cementOut' => $cementOut->id, 'user' => session('godown')]) }}" class="btn btn-sm btn-info">Edit</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
